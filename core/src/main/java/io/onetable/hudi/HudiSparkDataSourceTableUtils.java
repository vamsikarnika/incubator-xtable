...
public class HudiSparkDataSourceTableUtils {
  /**
  ...
  public static Map<String, String> getSparkTableProperties(
      ...
      OneSchema schema) {
    List<OneField> partitionCols = new ArrayList<>();
    List<OneField> dataCols = new ArrayList<>();
    Map<String, OneField> column2Field = new HashMap<>();
    ...
    for (String partitionName : partitionNames) {
      ...
      partitionCols.add(
          column2Field.getOrDefault(
              ...
              OneField.builder()
                  .name(partitionName)
                  .schema(OneSchema.builder().dataType(OneType.BYTES).isNullable(false).build())
                  .build()));
    }
    ...
    List<OneField> reOrderedFields = new ArrayList<>();
    reOrderedFields.addAll(dataCols);
    reOrderedFields.addAll(partitionCols);
    OneSchema reorderedSchema =
        OneSchema.builder()
    ...
    StructType sparkSchema = OneSchema2SparkSchemaUtils.convertToSparkSchema(reorderedSchema);
    ...
    String schemaString = sparkSchema.toString();
